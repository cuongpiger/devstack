...
TOP_DIR=$(cd $(dirname "$0") && pwd) # NOTE (cuongdm3); the directory path of the devstack directory

# Check for uninitialized variables, a big cause of bugs
NOUNSET=${NOUNSET:-} # NOTE (cuongdm3): do not care this options in my case
if [[ -n "$NOUNSET" ]]; then
  set -o nounset
fi
...
extract_localrc_section $TOP_DIR/local.conf $TOP_DIR/localrc $TOP_DIR/.localrc.auto # NOTE (cuongdm3): override the default env variables
...
# Phase: post-config, this phase will config the plugin osprofiler
...
if is_service_enabled mysql; then
  if [[ "$MYSQL_GATHER_PERFORMANCE" == "True" && "$MYSQL_HOST" ]]; then
    if [[ "$THIRD_SERVICES_IN_DOCKER" == "False" ]]; then
      echo ""
      echo ""
      ...
      if [ ! "$(docker ps -aqf "name=openstack-mysql" 2>/dev/null)" ]; then
      echo ""
      echo ""
      echo "Post-stack database query stats:"
