...
# `stack.sh` is an opinionated OpenStack developer installation. It installs and configures various combinations of
# Cinder, Glance, Horizon, Keystone, Nova, Neutron, and Swift.
#
# This script's options can be changed by setting appropriate environment variables. You can configure things like which
# git repositories to use, services to enable, OS images to use, etc.
#
# Default values are located in the `stackrc` file. If you are crafty you can run the script on multiple nodes using
# shared settings for common resources (eg., mysql or rabbitmq) and build a multi-node developer install.
#
# To keep this script simple we assume you are running on a recent Ubuntu (Bionic or newer), Fedora (F24 or newer), or
# CentOS/RHEL (7 or newer) machine. (It may work on other platforms but support for those platforms is left to those who
# added them to DevStack) It should work in a VM or physical server. Additionally, we maintain a list of deb and rpm
# dependencies and other configuration files in this repo.
#
# Learn more and get the most recent version at http://devstack.org

# ---------------------------------------------------------------------------------- SETTINGS DO NOT AFFECTS (SKIP THIS)

# Print the commands being run so that we can see the command that triggers an error. It is also useful for following
# along as the install occurs.
set -o xtrace

# Make sure custom grep options don't get in the way.
unset GREP_OPTIONS

# Devstack is written in bash, and many functions used throughout Devstack process text coming off a command (like the
# ip command) and do transforms using grep, sed, cut, awk on the strings that are returned. Many of these programs are
# internationalized, which is great for end users, but means that the strings that Devstack functions depend upon might
# not be there in other locales. We thus need to pin the world to an english basis during the runs.
#
# Previously we used the C locale for this, every system has it, and it gives us a stable sort order. It does however
# mean that we effectively drop unicode support.... boo! :(
#
# With Python-3 being more unicode aware by default, that's not the right option. While there is a C.utf8 locale, some
# distros are shipping it as C.UTF8 for extra confusingness. And it's support isn't super clear across distros. This is
# made more challenging when trying to support both out of the box distros, and the gate which uses diskimage builder to
# build disk images in a different way than the distros do.
#
# So... en_US.utf8 it is. That's existed for a very long time. It is a compromise position, but it is the least worse
# idea at the time of this comment.
#
# We also have to unset other variables that might impact LC_ALL taking effect.
unset LANG
unset LANGUAGE
LC_ALL=en_US.utf8
export LC_ALL
...
# Set the default permission maks, which determines the permissions assigned to newly created files and directories.
# In this case, it sets the mask to allow to read and write permissions for the owner of the files, and read-only
# permissions for the group and others.
umask 022

# Not all distros have sbin in PATH for regular users. `osc` will normally be installed at /usr/local/bin/openstack,
# so ensure /usr/local/bin is also in the path
PATH=$PATH:/usr/local/bin:/usr/local/sbin:/usr/sbin:/sbin

# Keep track of the DevStack directory, the TOP_DIR is the directory path of the devstack project
...
if [[ -r $TOP_DIR/.stackenv ]]; then # check the file exists and is readable
  rm $TOP_DIR/.stackenv
fi

# `stack.sh` keeps the list of deb and rpm dependencies, config, templates and other useful files in the `files`
# subdirectory
FILES=$TOP_DIR/files
if [ ! -d $FILES ]; then # check the directory `files` exists
  set +o xtrace
  echo "missing devstack/files"
  exit 1
fi

# `stack.sh` keeps function libraries here, make sure `$TOP_DIR/inc` directory is present
if [ ! -d $TOP_DIR/inc ]; then # check directory `inc` exists
  set +o xtrace
  echo "missing devstack/inc"
  exit 1
fi

# `stack.sh` keeps project libraries here, make sure `$TOP_DIR/lib` directory is present
if [ ! -d $TOP_DIR/lib ]; then # check the directory `lib` exists
  set +o xtrace
  echo "missing devstack/lib"
  exit 1
fi

# Check if run in POSIX shell, it does not affect on Ubuntu 22.04
...
# Provide a safety switch for devstack. If you do a lot of devstack, on a lot of different environments, you sometimes
# run it on the wrong box. This makes there be a way to prevent that.
...
# Import common functions from file `functions`
source $TOP_DIR/functions

# Import 'public' stack.sh functions
source $TOP_DIR/lib/stack

# Determine what system we are running on.  This provides `os_VENDOR`, `os_RELEASE`, `os_PACKAGE`, `os_CODENAME`
# and `DISTRO`
GetDistro
...
# Generate the .auto.localrc file if localrc file does not exist
rm -f $TOP_DIR/.localrc.auto
extract_localrc_section $TOP_DIR/local.conf $TOP_DIR/localrc $TOP_DIR/.localrc.auto

# If the file `stackrc` does not exist, die the execution of the script
...
# Warn users who aren't on an explicitly supported distro, but allow them to override check and attempt installation
# with `FORCE=yes ./stack`
...
# ------------------------------------------------------------------------------------------------------- LOCAL SETTINGS
# Make sure the proxy config is visible to sub-processes, just for more security.
export_proxy_variables

# Remove services which were negated in `ENABLED_SERVICES`, using the "-" prefix (e.g., "-rabbit") instead of calling
# disable_service().
...
# Ubuntu 22.04 does not need to do this
...
# Configure proper hostname, certain services such as rabbitmq require that the local hostname resolves correctly.
...
# If you have all the repos installed above already setup (e.g. a CI situation where they are on your image) you may
# choose to skip this to speed things up
SKIP_EPEL_INSTALL=$(trueorfalse False SKIP_EPEL_INSTALL) # currently it is False
if [[ $DISTRO == "rhel8" ]]; then
...
# NOTE(lyarwood): By default use a single hardcoded fixed_key across devstack deployments. This ensures the keys match
# across nova and cinder across all hosts.
...
# NOTE(clarkb): This must come after async_wait configure_neutron_nova because configure_neutron_nova modifies
# $NOVA_CONF and $NOVA_CPU_CONF as well. If we don't wait then these two ini updates race either other and can result in
# unexpected configs.
if is_service_enabled nova; then
...
# Start placement before any of the service that are likely to want to use it to manage resource providers.
if is_service_enabled placement; then
...
if is_service_enabled q-svc && [[ "$NEUTRON_CREATE_INITIAL_NETWORKS" == "True" ]]; then
  ...
  # Here's where plugins can wire up their own networks instead of the code in lib/neutron_plugins/services/l3
...
# This sleep is required for cinder volume service to become active and publish capabilities to cinder scheduler before
...
# NOTE (abhishekk): We need to start glance api service only after cinder service has started as on glance startup
# glance-api queries cinder for validating volume_type configured for cinder store of glance.
if is_service_enabled glance; then
...
# The default image is CirrOS, a small testing image which lets you login as root CirrOS has a `cloud-init` analog
# supporting login via keypair and sending scripts as userdata.
...
# This step also creates certificates for tenants and users, which is helpful in image bundle steps.
